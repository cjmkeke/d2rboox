...
public class Martial10 {

    public static void skillUpdate(int value, TextView textView, Context context){
        Type listType = new TypeToken<List<Martial10.MartialModel10>>() {}.getType();
        List<Martial10.MartialModel10> skill = JsonUtil.loadJSONFromAsset(context, "martial10.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(MartialUpdate.martialSkill10(
                                        String.valueOf(i),
                                        skill.get(i - 1).getOption1(), // ok
                                        skill.get(i - 1).getOption2(), // ok
                                        skill.get(i - 1).getOption5(),
                                        skill.get(i - 1).getOption3(),
                                        skill.get(i - 1).getOption4(),
                                        skill.get(i).getOption1(), // ok
                                        skill.get(i).getOption2(), // ok

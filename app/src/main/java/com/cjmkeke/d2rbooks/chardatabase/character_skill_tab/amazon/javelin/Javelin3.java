...
public class Javelin3 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        ...
        List<CombatModel10> skill = JsonUtil.loadJSONFromAsset(context, "javelin3.json", listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                spanned = Html.fromHtml(SkillJavelin.javelinSkill3_end, Html.FROM_HTML_MODE_LEGACY);
                textView.setText(spanned);
            } else if (value >= 1 && value < 21) {
                // Loop through values 0 to 19
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(JavelinUpdate.javelinSkill3(
                                        String.valueOf(i),

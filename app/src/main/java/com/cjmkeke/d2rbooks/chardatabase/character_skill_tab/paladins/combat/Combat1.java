...
public class Combat1 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context){
        ...
        Type listType = new TypeToken<List<Combat1.CombatModel10>>() {}.getType();
        List<Combat1.CombatModel10> skill = JsonUtil.loadJSONFromAsset(context, "combat1.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(CombatUpdate.combatSkill1(
                                        String.valueOf(i),

...
public class Martial8 {

    public static void skillUpdate(int value, TextView textView, Context context){
        Type listType = new TypeToken<List<Martial8.MartialModel10>>() {}.getType();
        List<Martial8.MartialModel10> skill = JsonUtil.loadJSONFromAsset(context, "martial8.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(MartialUpdate.martialSkill8(
                                        String.valueOf(i),

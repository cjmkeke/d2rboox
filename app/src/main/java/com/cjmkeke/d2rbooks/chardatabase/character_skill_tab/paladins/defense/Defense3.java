...
public class Defense3 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Defense3.DefenseModel10>>() {}.getType();
        List<Defense3.DefenseModel10> skill = JsonUtil.loadJSONFromAsset(context, "defense3.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(DefenseUpdate.defenseSkill3(
                                        String.valueOf(i),

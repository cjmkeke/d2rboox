...
public class Martial1 {


    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Martial1.MartialModel10>>() {}.getType();
        List<Martial1.MartialModel10> skill = JsonUtil.loadJSONFromAsset(context, "martial1.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(MartialUpdate.martialSkill1(
                                        String.valueOf(i),

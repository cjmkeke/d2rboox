...
public class CombatBarbarian10 {

    static final String JSON_FILE_NAME = "combat_babarian10.json";

    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<CombatBarbarian2.Models>>() {}.getType();
        List<CombatBarbarian2.Models> skill = JsonUtil.loadJSONFromAsset(context, JSON_FILE_NAME, listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                if (value == 20) {
                    spanned = Html.fromHtml(CombatBarbarianUpdate.combatSkill10_end(
                                    "20",
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(CombatBarbarianUpdate.combatSkill10(
                                        String.valueOf(i),

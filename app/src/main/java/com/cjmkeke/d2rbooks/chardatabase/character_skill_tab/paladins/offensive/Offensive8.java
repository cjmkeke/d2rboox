...
public class Offensive8 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Offensive8.OffensiveModel8>>() {}.getType();
        List<Offensive8.OffensiveModel8> skill = JsonUtil.loadJSONFromAsset(context, "offensive8.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(OffensiveUpdate.offensiveSkill8(
                                        String.valueOf(i),

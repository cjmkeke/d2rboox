...
public class Shape10 {
    ...
    static final String JSON_FILE_NAME = "shape10.json";

    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Shape2.Models>>() {}.getType();
        List<Shape2.Models> skill = JsonUtil.loadJSONFromAsset(context, JSON_FILE_NAME, listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                if (value == 20) {
                    spanned = Html.fromHtml(ShapeUpdate.shapeSkill10_end(
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(ShapeUpdate.shapeSkill10(
                                        String.valueOf(i),

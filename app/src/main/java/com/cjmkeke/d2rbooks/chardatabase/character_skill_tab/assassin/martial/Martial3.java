...
public class Martial3 {


    public static void skillUpdate(int value, TextView textView, Context context){
        Type listType = new TypeToken<List<Martial3.MartialModel10>>() {}.getType();
        List<Martial3.MartialModel10> skill = JsonUtil.loadJSONFromAsset(context, "martial3.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(MartialUpdate.martialSkill3(
                                        String.valueOf(i),

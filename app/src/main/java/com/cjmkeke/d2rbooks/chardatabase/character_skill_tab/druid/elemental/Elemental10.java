...
public class Elemental10 {

    static final String JSON_FILE_NAME = "elemental10.json";

    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Elemental2.Models>>() {}.getType();
        List<Elemental2.Models> skill = JsonUtil.loadJSONFromAsset(context, JSON_FILE_NAME, listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                if (value == 20) {
                    spanned = Html.fromHtml(ElementalUpdate.elementalSkill10_end(
                                    "20",
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(ElementalUpdate.elementalSkill10(
                                        String.valueOf(i),

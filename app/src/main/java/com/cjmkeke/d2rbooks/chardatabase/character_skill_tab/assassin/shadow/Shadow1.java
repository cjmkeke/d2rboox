...
public class Shadow1 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context){
        Type listType = new TypeToken<List<Shadow1.ShadowModel10>>() {}.getType();
        List<Shadow1.ShadowModel10> skill = JsonUtil.loadJSONFromAsset(context, "shadow1.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(ShadowUpdate.shadowSkill1(
                                        String.valueOf(i),

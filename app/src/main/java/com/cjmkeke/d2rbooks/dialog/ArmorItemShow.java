...
import com.cjmkeke.d2rbooks.databinding.ActivityArmorItemShowBinding;
...
public class ArmorItemShow extends Activity {

    private ActivityArmorItemShowBinding mBinding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = ActivityArmorItemShowBinding.inflate(getLayoutInflater());
        ...
        if (firebaseUser != null) {
            if (firebaseUser.getEmail().equals("cjmkeke@gmail.com")) {
                mBinding.tvArmorCommentCommit.setVisibility(View.VISIBLE);
                mBinding.etComment.setEnabled(true);
                mBinding.tvArmorCommentCommit.setOnClickListener(v -> {
                    databaseReference.child("item_data").child("armor").child(position).child("comment").setValue(mBinding.etComment.getText().toString());
                    finish();
                });
            } else {
                mBinding.tvArmorCommentCommit.setVisibility(View.GONE);
        ...
        databaseReference.child("item_data").child("armor").child(position).child("src").addValueEventListener(new ValueEventListener() {
            @Override

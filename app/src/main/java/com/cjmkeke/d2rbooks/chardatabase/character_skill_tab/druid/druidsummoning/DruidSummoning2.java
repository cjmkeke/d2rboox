...
public class DruidSummoning2 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences) {
        ...
        if (skill != null) {
            ...
            if (skill != null) {
                if (value == 20) {
                    spanned = Html.fromHtml(DruidSkillSummoning.druidSummoningSkill2_end, Html.FROM_HTML_MODE_LEGACY);
                    textView.setText(spanned);
                } else if (value >= 1 && value < 21) {
                    // Loop through values 0 to 19
                    for (int i = 0; i < 21; i++) {
                        if (value == i) {
                            spanned = Html.fromHtml(DruidSummoningUpdate.druidSummoningSkill2(
                                            String.valueOf(i),

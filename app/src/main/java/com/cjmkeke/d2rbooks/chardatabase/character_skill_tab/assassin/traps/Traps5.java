...
public class Traps5 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Traps5.TrapsModel10>>() {}.getType();
        List<Traps5.TrapsModel10> skill = JsonUtil.loadJSONFromAsset(context, "traps5.json", listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                spanned = Html.fromHtml(SkillTraps.trapsSkill5_end, Html.FROM_HTML_MODE_LEGACY);
                textView.setText(spanned);
            } else if (value >= 1 && value < 21) {
                // Loop through values 0 to 19
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(TrapsUpdate.trapsSkill5(
                                        String.valueOf(i),

...
public class Defense4 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Defense4.DefenseModel10>>() {}.getType();
        List<Defense4.DefenseModel10> skill = JsonUtil.loadJSONFromAsset(context, "defense2.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(DefenseUpdate.defenseSkill4(
                                        String.valueOf(i),

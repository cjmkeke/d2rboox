...
public class Martial9 {

    public static void skillUpdate(int value, TextView textView, Context context){
        Type listType = new TypeToken<List<Martial9.MartialModel10>>() {}.getType();
        List<Martial9.MartialModel10> skill = JsonUtil.loadJSONFromAsset(context, "martial9.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(MartialUpdate.martialSkill9(
                                        String.valueOf(i),

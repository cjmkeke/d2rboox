...
public class Fire10 {
    ...
    static final String JSON_FILE_NAME = "fire10.json";

    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Fire2.Models>>() {}.getType();
        List<Fire2.Models> skill = JsonUtil.loadJSONFromAsset(context, JSON_FILE_NAME, listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                if (value == 20) {
                    spanned = Html.fromHtml(FireUpdate.fireSkill10_end(
                                    "20",
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(FireUpdate.fireSkill10(
                                        String.valueOf(i),

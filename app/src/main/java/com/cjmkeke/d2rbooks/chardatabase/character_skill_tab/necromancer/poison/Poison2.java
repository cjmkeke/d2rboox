...
public class Poison2 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        ...
        List<TrapsModel10> skill = JsonUtil.loadJSONFromAsset(context, "poison2.json", listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                spanned = Html.fromHtml(SkillPoison.poisonSkill2_end, Html.FROM_HTML_MODE_LEGACY);
                textView.setText(spanned);
            } else if (value >= 1 && value < 21) {
                // Loop through values 0 to 19
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(PoisonUpdate.poisonSkill2(
                                        String.valueOf(i),

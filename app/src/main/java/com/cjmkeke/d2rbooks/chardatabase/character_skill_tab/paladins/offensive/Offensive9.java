...
public class Offensive9 {

    public static void skillUpdate(int value, TextView textView, Context context){
        Type listType = new TypeToken<List<Offensive9.OffensiveModel9>>() {}.getType();
        List<Offensive9.OffensiveModel9> skill = JsonUtil.loadJSONFromAsset(context, "offensive9.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(OffensiveUpdate.offensiveSkill9(
                                        String.valueOf(i),

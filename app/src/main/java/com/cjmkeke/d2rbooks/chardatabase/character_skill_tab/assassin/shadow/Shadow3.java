...
public class Shadow3 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context){
        Type listType = new TypeToken<List<Shadow3.ShadowModel10>>() {}.getType();
        List<Shadow3.ShadowModel10> skill = JsonUtil.loadJSONFromAsset(context, "shadow3.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(ShadowUpdate.shadowSkill3(
                                        String.valueOf(i),

...
public class Passive8 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        ...
        List<TrapsModel10> skill = JsonUtil.loadJSONFromAsset(context, "passive8.json", listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                spanned = Html.fromHtml(SkillPassive.passiveSkill8_end, Html.FROM_HTML_MODE_LEGACY);
                textView.setText(spanned);
            } else if (value >= 1 && value < 21) {
                // Loop through values 0 to 19
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(PassiveUpdate.passiveSkill8(
                                        String.valueOf(i),

...
public class Shadow6 {


    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Shadow6.ShadowModel10>>() {}.getType();
        List<Shadow6.ShadowModel10> skill = JsonUtil.loadJSONFromAsset(context, "shadow6.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(ShadowUpdate.shadowSkill6(
                                        String.valueOf(i),

...
public class Martial5 {


    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        Type listType = new TypeToken<List<Martial5.MartialModel10>>() {}.getType();
        List<Martial5.MartialModel10> skill = JsonUtil.loadJSONFromAsset(context, "martial5.json", listType);
        Spanned spanned;
        ...
        if (skill != null){
            if (value == 20) {
                ...
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(MartialUpdate.martialSkill5(
                                        String.valueOf(i),

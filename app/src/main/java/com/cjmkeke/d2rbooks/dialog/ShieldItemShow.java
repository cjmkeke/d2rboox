...
import com.cjmkeke.d2rbooks.databinding.ActivityShieldItemShowBinding;
...
public class ShieldItemShow extends Activity {

    private ActivityShieldItemShowBinding mBinding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding = ActivityShieldItemShowBinding.inflate(getLayoutInflater());
        ...
        if (firebaseUser != null) {
            if (firebaseUser.getEmail().equals("cjmkeke@gmail.com")) {
                mBinding.tvShieldCommentCommit.setVisibility(View.VISIBLE);
                mBinding.etComment.setEnabled(true);
                mBinding.tvShieldCommentCommit.setOnClickListener(v -> {
                    databaseReference.child("item_data").child("shield").child(position).child("comment").setValue(mBinding.etComment.getText().toString());
                    finish();
                });
            } else {
                mBinding.tvShieldCommentCommit.setVisibility(View.GONE);
        ...
        databaseReference.child("item_data").child("shield").child(position).child("src").addValueEventListener(new ValueEventListener() {
            @Override

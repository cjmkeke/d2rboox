...
public class NecromancerSummoning9 {
    ...
    public static void skillUpdate(int value, TextView textView, Context context, SharedPreferences sharedPreferences){
        ...
        List<Models> skill = JsonUtil.loadJSONFromAsset(context, "summoning9.json", listType);
        Spanned spanned;

        if (skill != null){
            if (value == 20) {
                spanned = Html.fromHtml(NecromancerSkillSummoning.summoningSkill9_end, Html.FROM_HTML_MODE_LEGACY);
                textView.setText(spanned);
            } else if (value >= 1 && value < 21) {
                // Loop through values 0 to 19
                for (int i = 0; i < 21; i++) {
                    if (value == i) {
                        spanned = Html.fromHtml(NecromancerSummoningUpdate.summoningSkill9(
                                        String.valueOf(i),
